...
function woo_wc_search_form( $form ) {
    ...
    <input type="search" results=5 autosave="'. esc_url(home_url( '/' )) .'" class="input-text" placeholder="'. esc_attr__( ' Поиск товара', 'woothemes' ) .'" value="' . get_search_query() . '" name="s" id="s" />
...
function mystile_recent_products() {
	...
	if (class_exists('woocommerce') && $woo_options[ 'woo_homepage_products' ] == "true" ) {
		echo '<h1>'.__('Последние товары', 'woothemes').'</h1>';
...
function woocommerce_cart_link() {
	...
	echo '<span class="contents">' . sprintf(_n('%d товар', '%d товара (ов)', $woocommerce->cart->get_cart_contents_count(), 'woothemes'), $woocommerce->cart->get_cart_contents_count()) . '</span>';
	?>
	</span>
	</a>
	</li>
